library(methylKit)
library(readxl)
setwd("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/2_bismark_filter/1_merged_CpG_covfiles/covfiles")
list.files()
...
directory="/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/2_bismark_filter/1_merged_CpG_covfiles/covfiles/"
...
meth=unite(normalized.myobj, min.per.group = 134L, destrand = F)
...
meth.168.20miss <- meth_nosex_norand

write.table(meth.168.20miss, file = NAME, quote = FALSE, sep = "\t",
            row.names = FALSE)
