...
listOfPackages <- c("ggplot2","reshape2","here", "ggforce", "matrixStats","viridis","readr","vegan","factoextra","readxl", "tidyverse","dplyr", "plotly", "GGally","ggrepel", "data.table", "ggVennDiagram", "ComplexHeatmap","missMDA")
...
#Create a contingency table of counts by pop1, pop2, and category
...
#Perform Chi-Square Test and view contingency table
...
First, I plot all three parental populations together, just to get an overview. After, I plot only RT comparisons in the top 1% of PST loci.
```{r}
...
## rustica-tytleri tests

I want to now plot only rustica-tytleri PST outliers and look at methylation values at these loci, and if they differ across the mean of each population. (Scripts to run significance tests are separate). This is to create a visual heatmap of all PST outliers and the methylation values in rustica and tytleri separately. 
```{r}
# --- Data Preparation ---
pst_rt <- top_1_pst_rgt_cat %>%
  filter(pop1 == "rustica.r", pop2 == "tytleri")

prop.df <- prop.RGT %>%
  dplyr::select(contains("RU_13_R_") | contains("_T_") | contains("region.id"))

population_groups <- list(Rr = "RU_13_R_", T = "_T_")
...
prop.rt <- cbind(prop.df, population_means) %>%
  dplyr::select(region.id, Rr, T)

pst_prop <- pst_rt %>%
  left_join(prop.rt, by = c("site.id" = "region.id"))

# This is now the primary data for the heatmap
numeric_matrix <- pst_prop %>%
  dplyr::select(matches("RU.*_R_"), matches("RU.*_T_"), T, Rr) %>%
...
# Draw the heatmap
draw(rt1, heatmap_legend_side = "bottom")
```
...
numeric_matrix_original <- pst_prop_islands %>%
  dplyr::select(T, Rr) %>% # Original order
  as.matrix()

# Reorder columns for horizontal flip if desired (Rr then G)
numeric_matrix_original <- numeric_matrix_original[, c("Rr", "T")]
...
row_split_labels <- c("H. r. rustica", "H. r. tytleri")
...
rt1 <- Heatmap(numeric_matrix_final, # Use the transposed matrix
...
pst_prop <- pst_rt %>%
  left_join(prop.rt, by = c("site.id" = "region.id")) # Ensure 'region' column is in pst_prop after this join
...
# The 'rows' of the transposed matrix are now G and Rr. No need to reorder them based on clustering.
# Define the new row split for the two rows (Rr, G)
row_split_labels <- c("H. r. rustica" = "Rr", "H. r. tytleri" = "T") # Correctly name the vector by row names
...
# --- Create Heatmap (Arguments swapped for transposed matrix) ---
rt1 <- Heatmap(numeric_matrix_transposed, # Use the transposed matrix
               cluster_rows = FALSE, # We don't want to cluster the populations (Rr, G)
...
# Draw the heatmap
draw(rt1, heatmap_legend_side = "right")
...
png("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/2_Parental_high_PST/1_Figure3_RT_Heatmap.png", width = 6, height = 2, units = "in", res = 500)
# Draw the heatmap and control the legend position and split order
draw(rt1, heatmap_legend_side = "right", cluster_column_slices = FALSE)
