...
listOfPackages <- c("ggplot2","reshape2","here", "ggforce", "matrixStats","viridis","readr","vegan","factoextra","readxl", "tidyverse","dplyr", "plotly", "GGally","ggpubr","ggrepel", "data.table", "missMDA", "gghalves", "broom", "ggdist")
...
pst_rt <- left_join(pst_rt, prop.hyb, by = c("site.id" = "region.id"))


# Remove "ME_" from the start of any column name in pst_rg
colnames(pst_rt) <- gsub("^ME_", "", colnames(pst_rt))
...
rt_admix <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/5_WGS/2_hybrid_index/2_newhybrids/RT_hyb/2_output_classes.txt", header=T)

#Define based on classes from New Hybrids
par.R <- rt_admix %>% filter(Assigned_Class == "R", grepl("_R_", Individual))
par.T <- rt_admix %>% filter(Assigned_Class == "T", grepl("_T_", Individual))
hyb.RT <- rt_admix %>% filter(Assigned_Class == "F1")

```

## Grab high PST loci for the RT comparison
```{r}
# Define population groups based on the id column of three dataframes
population_groups <- list(
  Rr = par.R$Individual,
  T = par.T$Individual,
  RT = hyb.RT$Individual
)
...
population_stats <- sapply(names(population_groups), function(pop) {
  ...
  cols <- intersect(ids, colnames(pst_rt))
  
  if (length(cols) == 0) return(NULL) # If no columns match, return NULL
  
  # Compute row means and variances
  means <- apply(pst_rt[, cols, drop = FALSE], 1, function(x) mean(x, na.rm = TRUE))
  vars  <- apply(pst_rt[, cols, drop = FALSE], 1, function(x) var(x, na.rm = TRUE))
...
pst_prop_rt <- cbind(pst_rt, population_stats_df)

pst_prop_rt <- pst_prop_rt %>% filter(!if_all(contains("_ADL_"), is.na))
#ONLY 1084 sites present in hybrids also with missing data below 20%

pst_prop_rt <- pst_prop_rt %>%
  dplyr::select(!contains("DE_") & !contains("CH_") & !contains("FI_") & !contains("_G_") & !contains("_RG_"))
...
T_names <- par.T$Individual
RT_names <- hyb.RT$Individual
...
individual_to_pop <- tibble(
  individual = c(R_names, T_names, RT_names),
  population = c(
    rep("R", length(R_names)),
    rep("T", length(T_names)),
    rep("RT", length(RT_names))
  )
)
```
...
long_data$population[is.na(long_data$population)] <- "RT"
```
...
Look at which sites are dominant for one parental, transgressive, or co-dominant. For this we will also need the men values of the populations
```{r}
...
final_tukey <- left_join(final_tukey, pst_prop_rt, by="site.id")
...
final_tukey <- final_tukey %>%
  mutate(
    type = case_when(
      ...
      `T-R_p_value_adjusted` < 0.05 & `T-RT_p_value_adjusted` < 0.05 & `RT-R_p_value_adjusted` > 0.05 ~ "rustica dominant",

      # G.dominant if R-G and RG-R are significant, but RG-G is not.
      `T-R_p_value_adjusted` < 0.05 & `RT-R_p_value_adjusted` < 0.05 & `T-RT_p_value_adjusted` > 0.05 ~ "tytleri dominant",

      # Transgressive if RG-R and RG-G are significant and RG is outside the R-G range.
      `RT-R_p_value_adjusted` < 0.05 & `T-RT_p_value_adjusted` < 0.05 & (RT > max(Rr, T) | RT < min(Rr, T)) ~ "transgressive",

      # Intermediate if RG-R and RG-G are significant and RG is between R and G.
      `RT-R_p_value_adjusted` < 0.05 & `T-RT_p_value_adjusted` < 0.05 & (RT > min(Rr, T) & RT < max(Rr, T)) ~ "co-dominant",
...
final_results_table <- final_tukey[,-c(18:84)]
final_results_table <- final_results_table[,-c(23:27)]
```

Save results
```{r}
write.table(final_results_table, "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/3_methylation_models/RT_hyb/RT_diff/0_RT_parental_hybrid_tukey_results.txt", sep= "\t", quote = FALSE, row.names = FALSE)

final_results <- read.delim("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/3_methylation_models/RT_hyb/RT_diff/0_RT_parental_hybrid_tukey_results.txt")
```

Run Fisher's exact test to detemine if it depends on being in an island, flank or site
```{r}
...
Now I would like to plot all 791 significantly different loci in PST outliers in a boxplot, and separate them into the groups identified from the ANOVA and Tukey Test.

```{r}
#Plot all loci
final_results <- left_join(pst_rt, final_results_table, by = "site.id")
...
final_results_long <- final_results_long %>%
  ...
  mutate(population = factor(population, levels = c("R", "RT", "T")))

final_results_long$type <- factor(final_results_long$type, levels = c("rustica dominant", "tytleri dominant", "co-dominant", "other", "not significant"))
...
final_results_long <- final_results_long %>%
  mutate(population = case_when(
    population == "R"  ~ "RUr",
    population == "T"  ~ "TY",
    population == "RT" ~ "hybrids",
    TRUE ~ population),
     population = factor(population, levels = c("RUr", "hybrids", "TY")))
...
ggsave("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/3_methylation_models/RT_hyb/RT_diff/0_RT_parental_hybrid_tukey_boxplots.png", plot = phenotype_plot, width = 12, height = 18)
```

Pull out one from each type to use as a subset in Figure 3.
R.dom chr26_4024119
T.dom chr07_36518714
Inter chr08_9797539

```{r}

final_results_long <- final_results_long %>% filter(site.id %in% c("chr26_4024119", "chr07_36518714", "chr08_9797539"))
...
save image 
```{r}
ggsave("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/3_methylation_models/RT_hyb/RT_diff/0_Figure3d_RT_boxplots.png", plot = sub_plot, width = 6, height = 3, units = "in", dpi = 500)
```
