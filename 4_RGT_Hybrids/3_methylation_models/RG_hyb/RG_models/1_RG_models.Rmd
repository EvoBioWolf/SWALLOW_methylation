...
global.anc <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/5_WGS/2_hybrid_index/1_introgress/1_RG_global_ancestry_informative_HI.txt", header=T)

local_SNP <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/5_WGS/2_hybrid_index/1_introgress/1_RG_local_nearest_SNP_maf1.txt", header=T)
...
tukey_results <- read.delim("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/3_methylation_models/RG_hyb/RG_diff/0_RG_parental_hybrid_tukey_results.txt", header=T)
...
## Grab significant loci from the ANOVA tests for the RG comparison
```{r}
...
cpg_loci <- cpg_loci %>%
  dplyr::select(!contains("DE_") & !contains("CH_") & !contains("FI_") & !contains("_T_") & !contains("_RT_") & !contains("_S_") & !contains("_TV_") & !contains("_E_") & !starts_with("RU_"))
...
I want to determine if cis- or trans- acting (or environmental/spatial) components influence methylation proportion at sites significantly different between the three groups (parent1, hybrids, parent2). Therefore, I want to run a model that take methylation prop ~ genome-wide HI + local HI (10kb surrounding focal region) + nearest_SNP (maf >0.1)
...
rg_admix <- rg_admix %>% dplyr::select(id.wolf, latitude, longitude, sex, proportion_rustica_ancestry)
...
#format correctly
local_SNP_for <- as.data.frame(t(local_SNP[,6:75]))
colnames(local_SNP_for) <- paste0(local_SNP$site_id, ".snp")
local_SNP_for$id <- rownames(local_SNP_for)

#Combine data
total_data <- left_join(meth.loci, rg_admix, by = c("id" = "id.wolf"))
...
#Keep only hybrids
total_data <- total_data %>% filter(grepl("_RG_",id))
```
...
# Ensure all columns that will be used are numeric.
# Ensure no values are outside (0,1)
RG.model.loci <- total_data %>%
  mutate(across(starts_with("chr"), ~ as.numeric(as.character(.))))

# Ensure predictors are numeric
RG.model.loci$longitude <- as.numeric(RG.model.loci$longitude)
RG.model.loci$proportion_rustica_ancestry <- as.numeric(RG.model.loci$proportion_rustica_ancestry)
RG.model.loci$all_vcf <- as.numeric(RG.model.loci$h)

# --- 1. Identify site.id columns ---
chr_columns <- names(RG.model.loci)[endsWith(names(RG.model.loci), ".meth")]
...
for (response_col in chr_columns) {
  ...
  if (local_predictor %in% colnames(RG.model.loci)) {
  ...
  model_fit <- tryCatch({
    glm(fml, family = quasibinomial(link = "logit"), data = RG.model.loci)
  }, error = function(e) {
...
#If  >1, it indicates overdispersion,  <1 it indicates underdispersion,  ≈1, the variance structure is similar to a standard binomial.
...
save results
```{r}
write.table(glm_summary, "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/3_methylation_models/RG_hyb/RG_models/1_RG_model_output_summary.txt", quote=F)
```
...
```{r}
#save results
write.table(glm.output.sig, "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/3_methylation_models/RG_hyb/RG_models/1_RG_model_output_sig.txt", sep = "\t", quote=F, row.names=F)
```
...
glm.distances.sig <- glm.distances.sig %>% 
   filter(distance != min(distance, na.rm = TRUE))

glm.full.output.snp <- glm.full.output.snp %>%
  mutate(term = str_remove(term, "\\.snp$"))

glm.distances <- left_join(glm.full.output.snp, local_SNP, by = c("term" = "site_id"))
...
```{r}
#save results
write.table(glm.distances.sig, "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/4_RGT_Hybrids/3_methylation_models/RG_hyb/RG_models/1_RG_model_output_sig_snp_dist.txt", sep = "\t", quote=F, row.names=F)
```
