...
cpgi.20miss <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_genome_files/3_CpG_genomic_regions/2_bedtools/0_cpgisland_cpgloci_overlap/cpgisland_cpgloci_output.85.20miss.update.bed", header=F)
...
#SHORES
#load bedtools output
cpg.shore.20miss <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_genome_files/3_CpG_genomic_regions/2_bedtools/0_cpgi_shores_shelves_overlap/cpgi.shores.85.20miss.update.bed", header=F)
...
prop.20miss <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/4_methyl_calling/0_all_sites/85.20miss/prop.85.merged.20miss.txt", header=T)

##load homer input 
homer.output.20miss <- read.delim("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_genome_files/3_CpG_genomic_regions/1_homer/homer.output.85.merged.20miss.txt")
...
homer.disagree <- homer.cpgi.prop.20miss %>%
                        ...
                        summarise(number_of_regions = n_distinct(Annotation))
...
homer.disagree.detail <- homer.cpgi.prop.20miss %>%
                              group_by(region.id) %>%
                              # Count the number of unique regions for each cpgi_id
                              summarise(number_of_regions = n_distinct(Detailed.Annotation))
...
count_annotations <- cpgi_values %>%
  group_by(cpgi.id) %>%
  summarise(n_unique_annotations = n_distinct(Annotation)) %>%
...
multiple_annotation_ids <- cpgi_values %>%
  group_by(cpgi.id) %>%
  filter(n_distinct(Annotation) > 1) %>%
  pull(cpgi.id) %>%
  unique()

#Filter the original data and find all pairs of annotations
annotation_lists <- cpgi_values %>%
  filter(cpgi.id %in% multiple_annotation_ids) %>%
  group_by(cpgi.id) %>%
  summarise(all_annotations = list(unique(Annotation))) %>%
  ungroup()
...
# 0.2819  0.7778  1.0000  0.8886  1.0000  1.0000 
...
sum(shore_values_test$n_cpgs <= 2)
...
annotation_shore_lists <- shore_values %>%
  filter(cpg.shore.id %in% shore_annotation_ids) %>%
  group_by(cpg.shore.id) %>%
  summarise(all_annotations = list(unique(Annotation))) %>%
  ungroup()
...
shore_values_distinct <- shore_values %>%
  group_by(cpg.shore.id, Annotation) %>%
...
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.3214  1.0000  1.0000  0.9310  1.0000  1.0000 
...
First, for all loci
```{r}
open_sea_values <- homer.cpgi.prop.20miss[,1:110] %>%
              filter(is.na(cpgi.id)) %>%
              filter(is.na(cpg.shore.id)) 
...
```{r}
prop.filtered.20miss <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/4_methyl_calling/0_all_sites/85.20miss/prop.85.merged.20miss.rmlowv.rmout.txt", header=T)
...
opensea_values_clean <- combined_data %>%
  select(region.id, n_cpgs, ends_with(".new"), everything(), -ends_with(".original"), -proportion.ave) %>%
  rename_with(~str_remove(., ".new"), ends_with(".new"))
...
I have run all of this on all CpG sites (n = 941,286). So now, I want to explore a bit of if there are still hyper- and hypo- methylated loci or islands/flanks that should be removed before downstream analysis.
...
write_delim(methylation.averages.filtered, file="/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/4_methyl_calling/1_CpG_islands/85.20miss/homer.cpgi.prop.85.merged.20miss.rmlowv.rmout.txt", delim="\t", quote_escape = "double")  
```
...
combined_cpgs_clean <- combined_cpgs %>%
  select(site.id, region.id, ends_with(".new"), everything(), -ends_with(".original"), -proportion.ave, -cpg.shore.id, -cpgi.id) %>%
  rename_with(~str_remove(., ".new"), ends_with(".new"))

# For homer.cpgi.prop.168.20miss
write_delim(combined_cpgs_clean, file="/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/4_methyl_calling/1_CpG_islands/85.20miss/homer.loci.prop.85.merged.20miss.rmlowv.rmout.txt", delim="\t", quote_escape = "double")  
```
...
write.table(category_df, file= "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/4_methyl_calling/1_CpG_islands/0_homer_loci_join_summary.txt", quote=F, sep = "\t")
...
write.table(category_cpgi, file= "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/4_methyl_calling/1_CpG_islands/0_homer_cpgi_join_summary.txt", quote=F, sep = "\t")
```
