...
load("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/3_sex_evaluation/2_168_sex_eval_BSobj.RData")
```
...
for (i in 1:100) {
  ...
  sex_random <- sample(sex_vector)
  # Create a dataframe
  sex_random <- data.frame(sex_random = sex_random)
  sex_random$sex_random <- as.factor(sex_random$sex_random)
  
  # Create design with sample names
  design <- cbind(sample_names, sex_random)
  
  # Define the design formula
  X = model.matrix(~sex_random, design)
  
  # Fit the linear model
  DMLfit = DMLfit.multiFactor(BSobj, design=design, formula=~sex_random)
...
write.table(all_pvals_combined,file="/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/3_sex_evaluation/all.pvals.txt", row.names = F, quote=F, sep="\t")
...
X = model.matrix(~sex.rrbs, design)
X
#fit the linear model
...
write.table(dmc.df,file='3_dmc.168.20miss.sexeval.txt',quote=F,sep='\t',row.names=F)
write.table(dmr.df,file='3_dmr.168.20miss.sexeval.txt',quote=F,sep='\t',row.names=F)
```
