...
# --- Working Directory ---
setwd("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/0_window_pst/")

# --- Input Data ---
cpgi <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/0_window_pst/0_pst_cpgi_10kb_c05.txt", header = TRUE)
shore <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/0_window_pst/0_pst_shore_10kb_c05.txt", header = TRUE)
opensea <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/0_window_pst/0_pst_opensea_10kb_c05.txt", header = TRUE)

pst_data <- rbind(cpgi,shore,opensea)
...
##Load homer to only keep islands
#homer.cpgi <- read.delim("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/4_methyl_calling/1_CpG_islands/168.20miss/homer.cpgi.prop.168.merged.20miss.rmlowv.rmout.txt")

#homer.cpgi <- homer.cpgi %>%  select(region.id,category)
#pst_data <- left_join(pst_data, homer.cpgi, by = c("site.id" = "region.id"))

##CHANGE DEPENDING ON WHICH REGION WILL BE CALCULATED
#pst_data <- pst_data %>%
...
write.table(final_results_gw, "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/2_Genome_Scan/2_PST_nearest_FST/2_simulation_results/1_PST_window_99_FST_99_1ksim_all_results.txt", quote=F, row.names=F)
