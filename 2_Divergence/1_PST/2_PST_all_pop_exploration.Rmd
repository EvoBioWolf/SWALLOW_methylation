...
setwd("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/0_site/variant/")

```

## PST Results

I want to look at possible patterns in the PST values across hybrid zones 
...
pst.quantiles.allsite <- pst.allsite %>%
  replace(is.na(.), 0) %>%
...
assign(Output_vector, pst.quantiles.allsite %>% drop_na(Pops) %>% dplyr::pull(site.id))

}

pst.list <- list(erythrogaster.vs.gutturalis = top1.erythrogaster.vs.gutturalis,
                 erythrogaster.vs.rustica.r = top1.erythrogaster.vs.rustica.r,
...
v2 <- ggvenn(pst.list, c("gutturalis.vs.rustica.r" , "gutturalis.vs.tytleri", "rustica.r.vs.tytleri"),
  fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4)
v2

v3 <- ggvenn(pst.list, c("gutturalis.vs.rustica.r" , "gutturalis.vs.rustica.eu"),
  fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4)
v3
```
...
pst.overlap <- data.frame(site.id = pst.quantiles.allsite$site.id,
                            count = (rowSums(!is.na(pst.quantiles.allsite))-1))

pst.overten <- pst.overlap %>%
                            filter(count > 5)

print(pst.overten)

histogram.cpgi <- ggplot(pst.overlap, aes(x = count)) +
...
pst.high <- pst.allsite %>% filter(gutturalis.vs.rustica.r.pst > .6)
```
...
homer.loci.20miss <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/0_Pre-processing/4_methyl_calling/1_CpG_islands/168.20miss/homer.loci.prop.168.merged.20miss.rmlowv.rmout.txt", header=T, sep="\t")


homer.all.pst.join <- left_join(pst.overten, homer.loci.20miss, by = "site.id")
```

```{r message= FALSE, warning=FALSE}
#Lets plot these aganist some information from homer to some genomic categories
# Plot boxplots for Gene.Type
gene_type_plot <- ggplot(homer.all.pst.join, aes(x = Gene.Type)) +
...
filtered_data_e <- homer.all.pst.join %>%
  group_by(Entrez.ID) %>%
  filter(n() > 10)
print(filtered_data_e)
# Plot histograms for Entrez.ID
entrez_id_plot <- ggplot(homer.all.pst.join, aes(x = Entrez.ID)) +
...
filtered_data <- homer.all.pst.join %>%
  group_by(Nearest.Unigene) %>%
  filter(n() > 10)
print(filtered_data)
# Plot histograms for Nearest.Unigene
nearest_unigene_plot <- ggplot(homer.all.pst.join, aes(x = Nearest.Unigene)) +
  geom_bar(fill = "#0097c3", color = "#333333") +
...
annotation_plot <- ggplot(homer.all.pst.join, aes(x = Annotation)) +
  geom_bar(fill = "#0097c3", color = "#333333") +
  labs(title = "Frequency of Sites by Annotation", x = "Annotation", y = "Frequency") +
  theme_minimal()
annotation_plot
# Plot histograms for Distance.to.TSS
distance_to_tss_plot <- ggplot(homer.all.pst.join, aes(x = Distance.to.TSS)) +
...
distance_to_tss_plot_zoom <- ggplot(homer.all.pst.join, aes(x = Distance.to.TSS)) +
  geom_histogram(fill = "#0097c3", color = "#333333") +
  xlim(c(-2000, 10000))+
  labs(title = "Frequency of Sites by Distance to TSS", x = "Distance to TSS", y = "Frequency") +
  theme_minimal()
...
summary(homer.all.pst.join$Distance.to.TSS)
```


Lets look at the mean methylation value across these sites per population. 
```{r message=FALSE, warning=FALSE}
# Calculate row means for columns containing pop info
mean_combined <- homer.all.pst.join %>%
  rowwise() %>%
