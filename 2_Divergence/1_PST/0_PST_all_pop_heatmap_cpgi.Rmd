...
setwd("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST_FST_gw/")
...
pst.allsite <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/0_cpgi_pst/0_pst_cpgi_c05.txt", header=T)
...
all_pst <- ggplot(pst.all, aes(x = pop1, y = pop2, fill = pst)) + 
  ...
  scale_fill_gradient2(
    ...
    midpoint = c(0.03),
    limits = c(0.024, 0.045),                   # force scale to 0–0.03
...
pst.cpgi <- cbind(pop.combo, t(pst.cpgi[1:21]))
colnames(pst.cpgi) <- c("pop1", "pop2", "pst")

pst.cpgi$pst <- as.numeric(pst.cpgi$pst)

pst.copy <- pst.cpgi %>% 
  transmute(pop1 = pst.cpgi$pop2, pop2 = pst.cpgi$pop1, pst = pst.cpgi$pst)
# Combine the two dataframes using bind_rows
pst.cpgi <- bind_rows(pst.cpgi, pst.copy)
...
# Plot PST values with the fixed order
cpgi_pst <- ggplot(pst.cpgi, aes(x = pop1, y = pop2, fill = pst)) + 
  geom_tile(
    data = subset(pst.cpgi, as.integer(pop1) < as.integer(pop2)), 
    aes(fill = pst),
    color = "black", size = 0.2   # thin black borders
  ) +
  geom_text(
  ...
  scale_fill_gradient2(
    low = "#8EC3E6", mid= "#e6f3f8", high = "red",
    midpoint = c(0.03),
    limits = c(0.024, 0.055),                   # force scale to 0–0.03
    breaks = c(0.03, 0.04, 0.05),            # only 3 breaks
    labels = scales::number_format(accuracy = 0.01)  # 2 decimal places
  ) +
  theme(
    axis.text.x = element_text(size = 10), 
...
I want to summarize across all loci and calculate one shore wide
...
pst.shore <- cbind(pop.combo, t(pst.shore[1:21]))
colnames(pst.shore) <- c("pop1", "pop2", "pst")

pst.shore$pst <- as.numeric(pst.shore$pst)

pst.copy <- pst.shore %>% 
  transmute(pop1 = pst.shore$pop2, pop2 = pst.shore$pop1, pst = pst.shore$pst)
# Combine the two dataframes using bind_rows
pst.shore <- bind_rows(pst.shore, pst.copy)
...
# Plot PST values with the fixed order
shore_pst <- ggplot(pst.shore, aes(x = pop1, y = pop2, fill = pst)) + 
  geom_tile(
    data = subset(pst.shore, as.integer(pop1) < as.integer(pop2)), 
    aes(fill = pst),
    color = "black", size = 0.2   # thin black borders
  ) +
  geom_text(
  ...
  scale_fill_gradient2(
    low = "#8EC3E6", mid= "#e6f3f8", high = "red",
    midpoint = c(0.03),
    limits = c(0.024, 0.05),                   # force scale to 0–0.03
    breaks = c(0.03, 0.04, 0.05),            # only 3 breaks
    labels = scales::number_format(accuracy = 0.01)  # 2 decimal places
  ) +
  theme(
...
##For all sites outside of CpGi and flanking regions
I want to summarize across all loci and calculate one metric

```{r message=FALSE, warning=FALSE}
#get mean PST across all sites for each comparison
pst.open <- opensea_pst %>%
    dplyr::summarize(
      across(2:22, mean, na.rm = TRUE),
      site_id = min(site.id, order_by = site.id),
      site_count = n()  # Count number of sites in each group
...
pst.open <- cbind(pop.combo, t(pst.open[1:21]))
colnames(pst.open) <- c("pop1", "pop2", "pst")

pst.open$pst <- as.numeric(pst.open$pst)

pst.copy.o <- pst.open %>% 
  transmute(pop1 = pst.open$pop2, pop2 = pst.open$pop1, pst = pst.open$pst)
# Combine the two dataframes using bind_rows
pst.open <- bind_rows(pst.open, pst.copy.o)
...
# Plot PST values with the fixed order
open_pst <- ggplot(pst.open, aes(x = pop1, y = pop2, fill = pst)) + 
  geom_tile(
    data = subset(pst.open, as.integer(pop1) < as.integer(pop2)), 
    aes(fill = pst),
    color = "black", size = 0.2   # thin black borders
  ) +
  geom_text(
...
Save images
```{r}
ggsave("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/1_Figure1d_PST_heatmap.png", plot = all_pst, width = 5, height = 4, units = "in", dpi = 500)

ggsave("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/1_SuppFig_PST_cpgi_heatmap.png", plot = cpgi_pst, width = 5, height = 4, units = "in", dpi = 500)

ggsave("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/1_SuppFig_PST_shore_heatmap.png", plot = shore_pst, width = 5, height = 4, units = "in", dpi = 500)

ggsave("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST/1_SuppFig_PST_opensea_heatmap.png", plot = open_pst, width = 5, height = 4, units = "in", dpi = 500)
```