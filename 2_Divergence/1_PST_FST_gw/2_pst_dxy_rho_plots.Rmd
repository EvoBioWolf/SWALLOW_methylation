...
all_regions_pst <- read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST_FST_gw/1_rho_dxy_per_category_results.txt", header=T)

all_site_pst <-read.table("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST_FST_gw/1_rho_dxy_per_site_results.txt", header=T)

all_site_pst <- all_site_pst %>% filter(!Annotation== "Mean")
all_site_pst$avg_fst <- "0"
all_site_pst <- all_site_pst %>%
  dplyr::select(avg_fst, pop_key, Pop1.x, Pop2.x, avg_dxy, Pop1.y, Pop2.y, Annotation, Mean)
...
correlation_df <- all_pst_sites %>%
  group_by(Annotation) %>%
  summarise(
    Spearman_Rho = cor.test(Mean, avg_dxy, method = "spearman")$estimate,
    P_value = cor.test(Mean, avg_dxy, method = "spearman")$p.value
  ) %>%
  ungroup()
...
correlation_df <- correlation_df %>% 
   mutate(Annotation = if_else(
    str_detect(Annotation, "_"), 
    Annotation, 
    paste0("all_", Annotation)
  ))
correlation_df <- correlation_df %>%
...
```{r}
ggsave("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST_FST_gw/Supp.Fig.I.PST.DXY.islands.png", plot = island.plot.size, width = 6, height = 7, units = "in", dpi = 500)
