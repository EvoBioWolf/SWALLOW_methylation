...
```{r}
fst.islands <- cbind(fst.islands[,3:4], dxy.gw[,c(1:2)])
...
# Create the plot
a <- ggplot(merged_df, aes(x = avg_fst, y = Mean)) +
  geom_point(size = 3) +
  stat_poly_line() +
  stat_poly_eq() +
  geom_smooth(method = "lm", se = T, color = "grey77", fill = "grey100") +
  labs(title = "Islands",x = "FST", y = "PST") +
...
cor <- cor(merged_df$Mean, merged_df$avg_fst, use = "pairwise.complete.obs", method= "spearman")
cor.test(merged_df$Mean, merged_df$avg_fst, use = "pairwise.complete.obs", method = "spearman")
...
merged_long <- merged_df %>%
  pivot_longer(cols = -c(Pop1.x, Pop1.y, Pop2.x, Pop2.y, avg_fst, pop_key), names_to = "Annotation", values_to = "Mean")
...
# Plot
p1 <- ggplot(merged_islands, aes(x = avg_fst, y = Mean)) +
  geom_point(size = 3) +
  stat_poly_line() +
  stat_poly_eq() +
...
For FST calculated from pixy
```{r}
fst.shore <- cbind(fst.shore[,3:4], dxy.gw[,c(1:2)])
...
# Create the plot
b <- ggplot(merged_df, aes(x = avg_fst, y = Mean)) +
  geom_point(size = 3) +
  stat_poly_line() +
  stat_poly_eq() +
  geom_smooth(method = "lm", se = T, color = "grey77", fill = "grey100", fill_alpha = 0.9) +
  labs(title = "Flanks",x = "FST", y = "PST") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
        plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
...
# Plot
p1 <- ggplot(merged_shore, aes(x = avg_fst, y = Mean)) +
  geom_point(size = 3) +
  stat_poly_line() +
  stat_poly_eq() +
  #geom_text(data = subset(merged_df, corrected_fst > 0.4), aes(label = site.id), vjust = -0.5, hjust = 1) +
...
For FST calculated from pixy
```{r}
fst.opensea <- cbind(fst.opensea[,3:4], dxy.gw[,c(1:2)])
...
# Create the plot
c <- ggplot(opensea_df, aes(x = avg_fst, y = Mean)) +
  geom_point(size = 3) +
  stat_poly_line() +
  stat_poly_eq() +
  geom_smooth(method = "lm", se = T, color = "grey77", fill = "grey100", fill_alpha = 0.9) +
...
cor <- cor(opensea_df$Mean, opensea_df$avg_fst, use = "pairwise.complete.obs", method= "spearman")
cor.test(opensea_df$Mean, opensea_df$avg_fst, use = "pairwise.complete.obs", method = "spearman")
...
# Plot
p1 <- ggplot(merged_opensea, aes(x = avg_fst, y = Mean)) +
  geom_point(size = 3) +
  stat_poly_line() +
  stat_poly_eq() +
  #geom_text(data = subset(merged_df, corrected_fst > 0.4), aes(label = site.id), vjust = -0.5, hjust = 1) +
...
save results
```{r}
ggsave("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST_FST_gw/SuppFig_PST_FST_regional.png", arrangeGrob(a, b, c, ncol = 3), width = 12, height = 4, units = "in", dpi = 500)

all_regions_pst <- rbind(merged_islands, merged_shore, merged_opensea)

write.table(all_regions_pst, "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowRRBS/2_Divergence/1_PST_FST_gw/1_rho_FST_per_category_results.txt", quote=F, row.names=F)
```
