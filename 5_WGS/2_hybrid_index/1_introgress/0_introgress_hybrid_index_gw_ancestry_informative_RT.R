...
vcf <- read.vcfR("/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowWGS_168/2_hybrid_index/0_ancestry_informative/RT.ancestry.informative.fst8.vcf.gz")

# Load the table containing sample names and population information.
sample_names_rt <- read.table("/dss/dsshome1/lxc0B/ra52qed/gitlab/swallow.projects/scripts/0_sample.lists/sample.names.60.RThyb.sub.pop.txt", header=T)
...
T_idx <- c(50:54,56,59,60)     # columns 1–12 = T parental samples
R_idx <- c(1:4,7:12)    # columns 59–70 = R parental samples

# Function to calculate AF given a genotype matrix
calc_AF <- function(geno_mat) {
  # G parental AF
  T.af <- (rowSums(geno_mat[, T_idx, drop=FALSE], na.rm = TRUE) /
             rowSums(!is.na(geno_mat[, T_idx, drop=FALSE]))) / 2
  ...
  data.frame(T.af = T.af, R.af = R.af)
}
...
find_fixed_snps <- function(geno_mat, af_df, cutoff = 0.4) {
  ...
  diff <- abs(af_df$T.af - af_df$R.af)
...
gen.mat <- fixed_subsets$all_vcf$gen.mat[, 1:60]
...
T.af <- fixed_subsets$all_vcf$gen.mat[,c(50:54,56,59,60)]
R.af <- fixed_subsets$all_vcf$gen.mat[,c(1:4,7:12)]
...
plot_data <- hi.het.hyb %>%
  ...
  mutate(color_group = case_when(
    grepl("_RT_", sample) ~ "RT",  # Check for RG first so it isn't caught by R or G alone
    grepl("_T_",  sample) ~ "T",
    grepl("_R_",  sample) ~ "R",
    TRUE                  ~ "Other"
  ))
...
# --- 12. Save Files --------------
write.table(hi.het.hyb, "/dss/dsslegfs01/pr53da/pr53da-dss-0034/projects/2021_SwallowWGS_168/2_hybrid_index/1_introgress/1_RT_global_ancestry_informative_HI.txt", quote=F)
